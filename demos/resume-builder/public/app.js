...
async function initHealth() {
  try {
    ...
    if (!data?.compiler?.available) {
      ...
      status('PDF generation disabled: pdflatex is not installed on the server.', 'error');
      return;
    }
    status('Ready. LaTeX compiler is available.', 'ok');
  } catch (_error) {
    status('Ready. Could not verify compiler health, but you can still try compiling.', '');
  }
}
